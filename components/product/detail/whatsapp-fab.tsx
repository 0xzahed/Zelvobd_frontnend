...
export function WhatsAppFab({ number }: { number: string }) {
  ...
  useEffect(() => {
    ...
    const checkStatus = () => {
      ...
      const bdTime = new Date(bdTimeString)
      const hours = bdTime.getHours()
      const minutes = bdTime.getMinutes()
      
      // Active from 8:01 AM to 11:59 PM
      // Inactive from 12:00 AM to 8:00 AM
      const active = hours > 8 || (hours === 8 && minutes > 0)
      setIsActive(active)
    }
  ...
  return (
    ...
    >
      ...
      {mounted && (
        <span
          className={`absolute right-0 top-0 h-3.5 w-3.5 rounded-full border-2 border-white dark:border-card ${
