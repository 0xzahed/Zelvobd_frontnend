...
export default function FeaturesTab({ register, control }: { register: any; control: any }) {
  const {
    ...
    append: addCard,
    remove: removeCard,
  } = useFieldArray({ control, name: 'featureCards' });
  const {
    fields: videoCards,
    append: addVideoCard,
    remove: removeVideoCard,
  } = useFieldArray({ control, name: 'videoSection.cards' });
  const {
    fields: points,
    append: addPoint,
    remove: removePoint,
  } = useFieldArray({ control, name: 'bulletPointsSection.points' });
    ...
    <div className='space-y-10'>
      ...
      <div className='space-y-4'>
        <div className='flex items-center justify-between border-b pb-2'>
          <h3 className='text-lg font-semibold'>
            Mini Feature Cards (e.g. Free Delivery, 7 Days Return)
          </h3>
          <button
            type='button'
            onClick={() => addCard({ icon: 'check', title: '' })}
            ...
            <div key={field.id} className='p-4 border rounded-md relative group'>
              ...
              <div className='grid grid-cols-2 gap-3'>
                <div>
                  ...
                  <Input
                    {...register(`featureCards.${index}.icon`)}
                    placeholder='e.g. truck, shield, rotate-ccw'
                  />
                </div>
                <div>
                  <Label>Title</Label>
      ...
      <div className='space-y-4'>
        ...
        <div className='grid grid-cols-2 gap-4'>
          <div>
            <Label>Caption</Label>
            <Input {...register('videoSection.caption')} placeholder='Caption' />
          </div>
          <div>
            <Label>Youtube Video Link</Label>
            <Input {...register('videoSection.videoLink')} placeholder='https://youtube.com/...' />
          </div>
          <div>
            <Controller
        ...
        <div className='mt-4'>
          <div className='flex items-center justify-between mb-2'>
            ...
            <button
              type='button'
              onClick={() => addVideoCard({ icon: '', title: '', subtitle: '', image: '' })}
              className='text-sm text-primary flex items-center gap-1 hover:underline'
            >
              <Plus className='h-4 w-4' /> Add Card
            </button>
      ...
      <div className='space-y-4'>
        ...
        <div className='grid grid-cols-3 gap-4'>
          <div>
            <Label>Caption</Label>
            <Input {...register('bulletPointsSection.caption')} />
          </div>
          <div>
            <Label>Title</Label>
            <Input {...register('bulletPointsSection.title')} />
          </div>
          <div>
            <Label>Subtitle</Label>
            <Input {...register('bulletPointsSection.subtitle')} />
          </div>
        </div>
        <div className='space-y-2 mt-2'>
          {points.map((field: any, index) => (
            <div key={field.id} className='flex gap-2'>
              <Input
                {...register(`bulletPointsSection.points.${index}.value`)}
