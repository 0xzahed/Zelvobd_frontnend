...
export function CartBottomSheet({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    const loadDetails = async () => {
      const targets = items
        .map((item) => products.find((prod) => prod.id === item.productId))
        .filter(Boolean)
        .filter((p) => p && !detailMap[p.id]) as Product[];
    ...
    if (open && items.length > 0 && products.length > 0) {
      void loadDetails();
    }
  ...
  }, [open, items, products, detailMap]);

  const enriched = items.map((item) => {
    const p = detailMap[item.productId] || products.find((prod) => prod.id === item.productId)
  ...
  const getActiveVariant = (item: any) => {
    const p = item.product;
    const norm = (val?: string) => (val || '').trim().toLowerCase();
    const itemColor = norm(item.color);
    const itemSize = norm(item.storage);
    return p.variants?.find((v: any) => norm(v.color) === itemColor && v.size.split(',').map(norm).includes(itemSize)) 
      || p.variants?.find((v: any) => norm(v.color) === itemColor) 
      || p.variants?.find((v: any) => v.size.split(',').map(norm).includes(itemSize))
      || p.variants?.[0];
  };

  const subtotal = enriched.reduce((s, i) => {
    const v = getActiveVariant(i);
    return s + getVariantPrice(i.product, v) * i.quantity;
  }, 0);
  ...
  return (
    ...
    >
      ...
      />
      <div className='absolute bottom-0 left-0 right-0 flex max-h-[50dvh] flex-col rounded-t-3xl bg-card'>
        ...
        <div className='min-h-0 flex-1 overflow-y-auto px-5'>
          {enriched.length === 0 ? (
            <div className='flex h-full min-h-30 flex-col items-center justify-center gap-2 py-6 text-center'>
            ...
            <ul className='divide-y divide-border'>
              {enriched.map((item) => {
                ...
                const itemColor = norm(item.color);
                const itemSize = norm(item.storage);
                const variant = getActiveVariant({ product: p, color: item.color, storage: item.storage });
                const imageToUse = variant?.image || p.images?.[0] || '/placeholder.svg';
                return (
                  <li
        ...
        <div className='border-t border-border/60 px-5 py-3'>
          <div className='flex items-center justify-between pb-3'>
            ...
            <span className='text-base font-bold text-foreground'>{formatBDT(subtotal)}</span>
          </div>
          <Link
            href='/cart'
            onClick={onClose}
