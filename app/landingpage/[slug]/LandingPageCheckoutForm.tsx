...
export default function LandingPageCheckoutForm({ landingPageId, checkoutData }: { landingPageId: string, checkoutData: any }) {
  ...
  if (submitted) {
    return (
      <div className="bg-white rounded-3xl p-8 shadow-xl text-center max-w-lg mx-auto">
        ...
        <p className="text-slate-500">Your order has been successfully placed. Our team will contact you shortly to confirm the delivery.</p>
      </div>
    );
  }

  return (
    <div className="bg-white rounded-3xl p-6 md:p-8 shadow-xl max-w-xl mx-auto border-4" style={{ borderColor: 'var(--lp-cta)' }}>
      <div className="mb-6 flex items-center justify-between gap-3 rounded-2xl bg-slate-50 p-4 border border-slate-100">
        <div className="min-w-0 text-left">
          <div className="font-bold text-slate-800 text-lg truncate">{checkoutData.productName || 'Special Product'}</div>
      ...
      <form onSubmit={handleSubmit} className="space-y-4 text-left">
        ...
        <div>
          <label className="block text-sm font-semibold text-slate-700 mb-1">আপনার নাম (Your Name) <span className="text-red-500">*</span></label>
          <input 
            type="text" 
            required 
            value={form.name}
        ...
        <div>
          <label className="block text-sm font-semibold text-slate-700 mb-1">মোবাইল নাম্বার (Mobile Number) <span className="text-red-500">*</span></label>
          <input 
            type="tel" 
            required 
            value={form.phone}
        ...
        <div>
          <label className="block text-sm font-semibold text-slate-700 mb-1">সম্পূর্ণ ঠিকানা (Full Address) <span className="text-red-500">*</span></label>
          <textarea 
            required 
            value={form.address}
        ...
        {checkoutData.deliveryText && (
           <div className="text-center text-sm font-semibold bg-[var(--lp-highlight)] text-[var(--lp-navy)] py-2 rounded-lg mt-2">
             {checkoutData.deliveryText}
           </div>
        )}
