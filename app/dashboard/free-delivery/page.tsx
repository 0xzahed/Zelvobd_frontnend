...
import { Search, CheckSquare, Truck, Loader2 } from "lucide-react"
import { DashPage, DashHeader, DashPanel, DashLoading } from "@/dashboard/components/dash-ui"
import { Switch } from "@/components/ui/switch"
import { useFreeDeliveryAdmin, useUpdateFreeDeliveryCampaign, useUpdateFreeDeliveryProducts } from "@/src/hooks/api/useFreeDelivery"
import { useCategories, useSubCategories } from "@/src/hooks/api/useCategories"
import { useProducts } from "@/src/hooks/api/useProducts"

export default function DashboardFreeDeliveryPage() {
  const { data: adminData, isLoading } = useFreeDeliveryAdmin()
  const updateCampaignMutation = useUpdateFreeDeliveryCampaign()
  const updateProductsMutation = useUpdateFreeDeliveryProducts()
  ...
  if (isLoading) return <DashPage><DashLoading label="Loading free delivery data..." /></DashPage>

  if (!adminData) {
    return (
      <DashPage>
        <DashHeader title="Free Delivery" subtitle="Manage free delivery products" />
        <DashPanel>
          <div className="flex flex-col items-center justify-center gap-2 py-16 text-center">
            <p className="text-base font-medium text-foreground">Error loading free delivery data</p>
  ...
  return (
    <DashPage>
      <DashHeader
        title="Free Delivery"
        subtitle={`${adminData.sources.products.length} products with free delivery`}
      />
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">Toggle to activate free delivery campaign</p>
            </div>
          </div>
          <div className="flex items-center gap-3">
      ...
      <DashPanel className="p-5">
        <div className="mb-4 flex items-center justify-between border-b border-border/40 pb-4">
          <div>
            ...
            <p className="text-xs text-muted-foreground">Select products to offer with free delivery</p>
          </div>
          <button
            onClick={handleSave}
