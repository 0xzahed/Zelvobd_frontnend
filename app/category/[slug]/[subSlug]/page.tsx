  ...
  return (
    <AppShell>
      ...
      <div className="py-4 md:py-6">
        ...
        {items.length > 0 ? <div className="grid grid-cols-2 gap-3 md:grid-cols-4 md:gap-4 lg:grid-cols-5">{items.map((p) => <ProductCard key={p.id} product={p} />)}</div> : <div className="rounded-2xl bg-card p-10 text-center"><p className="text-sm text-muted-foreground">No products in this sub-category yet.</p></div>}
      </div>
    </AppShell>
  )
}
