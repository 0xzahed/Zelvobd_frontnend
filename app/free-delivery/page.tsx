...
export default function FreeDeliveryPage() {
  ...
  const items = products.filter((p) => p.isFreeDelivery)

  return (
    <AppShell>
      <BackHeader title="Free Delivery" />
      <div className="space-y-4 py-4 md:space-y-6 md:py-6">
        <section className="space-y-3">
          <div className="flex items-center gap-2">
            <Truck className="h-5 w-5 text-[#3B6CF4] md:h-6 md:w-6" />
            <h2 className="text-base font-bold text-foreground md:text-xl">All Free Delivery Items</h2>
          </div>
          ...
          {items.length > 0 ? (
            ...
            <div className="rounded-2xl bg-card p-10 text-center">
              <p className="text-sm text-muted-foreground">No free delivery items yet.</p>
            </div>
          )}
        </section>
      </div>
