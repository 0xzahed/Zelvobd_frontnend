...
import { ImagePlus, Plus, Search, Trash2, Pencil, Eye, X } from "lucide-react"
import { useCategories, useCreateSubCategory, useUpdateSubCategory, useDeleteSubCategory } from "@/src/hooks/api/useCategories"
  ...
  const submit = () => {
    ...
    if (editingSub) {
      ...
      createMutation.mutate({
        categoryId,
        sub: { id: `sub-${Date.now()}`, name: n, slug: n.toLowerCase().replace(/\s+/g, "-"), image: image || "/placeholder.svg" },
      })
    }
    closeModal()
  }
            ...
            <table className="w-full text-left text-sm">
              <thead>
                <tr className="border-b border-border/40 bg-surface/50 text-muted-foreground">
                  <th className="px-5 py-3 text-xs font-semibold uppercase tracking-wide">Sub Category</th>
                  <th className="px-5 py-3 text-xs font-semibold uppercase tracking-wide">Parent Category</th>
              ...
              <tbody>
                ...
                {paginated.map((s: any) => (
                  <tr key={s.id} className="border-b border-border/30 transition hover:bg-surface/50">
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-3">
                        <div className="relative h-10 w-10 shrink-0 overflow-hidden rounded-lg border border-border/40 bg-surface">
                          <Image src={s.image || "/placeholder.svg"} alt={s.name} fill className="object-cover" unoptimized />
                        </div>
                    ...
                    <td className="px-5 py-3.5 text-muted-foreground">{formatRelativeTime(s.createdAt)}</td>
                    <td className="px-5 py-3.5">
          ...
          {paginated.map((s: any) => (
            ...
            >
              ...
              <div className="flex flex-1 flex-col p-3.5">
                ...
                <p className="mt-2 text-xs text-muted-foreground">Created {formatRelativeTime(s.createdAt)}</p>
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/40 p-4" role="dialog" aria-modal="true" onClick={closeModal}>
          <div className="relative w-full max-w-md rounded-2xl bg-card p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
            ...
            <h3 className="text-center text-base font-bold text-foreground">{editingSub ? "Edit Sub Category" : "Add New Sub Category"}</h3>
            <div className="mx-auto mt-4 h-px w-full bg-border/60" />

            <div className="mt-5">
              <label className="mb-1.5 block text-xs font-semibold uppercase tracking-wide text-muted-foreground">Parent Category</label>
              <select
                value={categoryId}
