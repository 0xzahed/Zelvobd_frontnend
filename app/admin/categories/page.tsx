...
import { useCategories, useCreateCategory, useUpdateCategory, useDeleteCategory } from "@/src/hooks/api/useCategories"
...
export default function DashboardCategoriesPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return categories.filter((c: any) => c.name.toLowerCase().includes(lc) || c.slug.includes(lc))
  }, [categories, q])
  ...
  const submit = () => {
    ...
    if (hasDuplicate) {
      notify.error({ title: "Category already exists", message: "Please use a different category name." })
      return
    }
    if (!editingId && !image) {
      notify.error({ title: "Image is required", message: "Please upload a category image before submitting." })
      return
    }
  ...
  if (isLoading) {
    return (
      <DashPage>
        <DashHeader title="Categories" subtitle="Manage your product categories" />
        <DashLoading label="Loading categories..." />
      </DashPage>
    )
  }
  ...
  return (
    <DashPage>
            ...
            <table className="w-full text-left text-sm">
              <thead>
                <tr className="border-b border-border/40 bg-surface/50 text-muted-foreground">
                  ...
                  <th className="px-5 py-3 text-xs font-semibold uppercase tracking-wide">Sub Categories</th>
              ...
              <tbody>
                ...
                {paginated.map((c: any) => (
                  <tr key={c.id} className="border-b border-border/30 transition hover:bg-surface/50">
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-3">
                        <div className="relative h-10 w-10 shrink-0 overflow-hidden rounded-lg border border-border/40 bg-surface">
                          <Image src={c.image || "/placeholder.svg"} alt={c.name} fill className="object-cover" unoptimized />
                        </div>
                        <span className="font-medium text-foreground">{c.name}</span>
                      </div>
                    </td>
                    <td className="px-5 py-3.5 text-muted-foreground">/{c.slug}</td>
                    <td className="px-5 py-3.5 text-muted-foreground">{(c.subCategories || []).length}</td>
                    <td className="px-5 py-3.5 text-muted-foreground">{formatRelativeTime(c.createdAt)}</td>
                    <td className="px-5 py-3.5">
            ...
            >
              <div className="relative h-40 w-full overflow-hidden bg-white">
                <Image
                  ...
                  unoptimized
                />
                <div className="absolute right-2 top-2 rounded-full bg-card/90 px-2 py-0.5 text-[10px] font-bold text-muted-foreground shadow-sm">
              ...
              <div className="flex flex-1 flex-col p-3.5">
                ...
                <p className="mt-2 text-xs text-muted-foreground">Created {formatRelativeTime(c.createdAt)}</p>
                <div className="mt-3 grid grid-cols-3 gap-2">
                  ...
                  <button
                    onClick={() => void handleDelete(c)}
                    className="flex h-8 items-center justify-center gap-1.5 rounded-lg bg-red-50 text-xs font-semibold text-red-500 transition hover:bg-red-500 hover:text-white"
                  >
      ...
      {/* Create/Edit Modal */}
      {showModal && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/40 p-4" role="dialog" aria-modal="true" onClick={closeModal}>
          <div className="relative w-full max-w-md rounded-2xl bg-card p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
            <button type="button" aria-label="Close" onClick={closeModal} className="absolute right-4 top-4 text-muted-foreground hover:text-foreground">
              <X className="h-4 w-4" />
            </button>
            <h3 className="text-center text-base font-bold text-foreground">{editingId ? "Edit Category" : "Add New Category"}</h3>
            ...
            <div className="mt-4">
              <label className="mb-2 block text-xs font-semibold uppercase tracking-wide text-muted-foreground">Image Upload</label>
              <input ref={imgInputRef} type="file" accept="image/*" className="sr-only" onChange={(e) => pickImage(e.target.files?.[0])} />
              <button type="button" onClick={() => imgInputRef.current?.click()} className="flex w-full flex-col items-center justify-center gap-1 overflow-hidden rounded-xl border-2 border-dashed border-primary/40 bg-secondary/30 py-6 text-center transition hover:bg-secondary">
                {image ? (
                  <img src={image || "/placeholder.svg"} alt="Preview" className="mx-auto max-h-28 rounded-lg object-contain" />
                ) : (
                  <>
      ...
      {/* View Modal */}
      {viewOpen && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/40 p-4" role="dialog" aria-modal="true" onClick={() => setViewOpen(false)}>
          <div className="relative w-full max-w-md rounded-2xl bg-card p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
            <button type="button" aria-label="Close" onClick={() => setViewOpen(false)} className="absolute right-4 top-4 text-muted-foreground hover:text-foreground">
              <X className="h-4 w-4" />
            </button>
            <h3 className="text-center text-base font-bold text-foreground">Category Details</h3>
            <div className="mx-auto mt-4 h-px w-full bg-border/60" />

            {viewLoading && <p className="py-10 text-center text-sm text-muted-foreground">Loading details...</p>}
            ...
            {!viewLoading && viewCategory && (
              <div className="mt-5 space-y-4">
                <div className="mx-auto h-36 w-36 overflow-hidden rounded-xl border border-border/60 bg-secondary">
                  <img src={viewCategory.imageUrl || "/placeholder.svg"} alt={viewCategory.title} className="h-full w-full object-cover" />
                </div>
                <div className="space-y-2 rounded-xl border border-border/60 bg-secondary/30 p-4 text-sm">
                  <p className="text-foreground"><span className="mr-2 font-semibold">Title:</span>{viewCategory.title}</p>
                  <p className="text-foreground"><span className="mr-2 font-semibold">Slug:</span>/{viewCategory.slug}</p>
                  <p className="text-muted-foreground"><span className="mr-2 font-semibold text-foreground">Created:</span>{viewCategory.createdAt ? new Date(viewCategory.createdAt).toLocaleString() : "N/A"}</p>
                  <p className="text-muted-foreground"><span className="mr-2 font-semibold text-foreground">Updated:</span>{viewCategory.updatedAt ? new Date(viewCategory.updatedAt).toLocaleString() : "N/A"}</p>
                </div>
              </div>
            )}
          </div>
