...
export function DashboardSidebar({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <>
      ...
      <aside
        className={cx(
          'fixed inset-y-0 left-0 z-40 flex w-62.5 flex-col border-r border-border/40 bg-surface-elevated transition-transform duration-300 ease-out md:translate-x-0',
      ...
      >
        ...
        {/* Nav */}
        <nav className='flex-1 overflow-y-auto px-3 pb-4 [&::-webkit-scrollbar]:hidden scrollbar-none'>
          {MENU.map((section) => (
                    ...
                    return (
                      <li key={item.label}>
                        ...
                        >
                          ...
                          />
                          <span className={cx('flex-1', active ? 'text-foreground' : '')}>
                            {item.label}
                          </span>
                          {item.badge && (
                  ...
                  return (
                    <li key={item.label}>
                      ...
                      >
                        ...
                        />
                        <span
                          className={cx('flex-1 text-left', groupActive ? 'text-foreground' : '')}
                        >
                          {item.label}
                        </span>
                        <ChevronRight
